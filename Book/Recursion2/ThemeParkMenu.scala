...
case class EmployeeData(name:String, rides:List[String], daysPerWeek:Int)
...
def parseEmployeeData(node:Node):EmployeeData = {
  ...
  val dpw = (node \ "@daysPerWeek").text.toInt
  EmployeeData(name, rides, dpw)
}

def employeeToXML(ed:EmployeeData):Node = {
  <employee name={ed.name} daysPerWeek={ed.daysPerWeek.toString}>
...
def recursiveBuildWeeklySchedule {
  ...
  def recurByWorker(daysLeft:List[String], workerAvail:List[WorkerDays], schedule:List[WorkerAssigns], workersLeft:List[String], ridesNeedingOps:List[String]) {
      ...
      for (ride <- worker.rides) {
        ridesNeedingOps.indexOf(ride) match {
          ...
          case i =>
            ...
            val newSchedule = schedule.head.copy(workerRide = (worker.name, ride) :: schedule.head.workerRide) :: schedule.tail
            recurByWorker(daysLeft, newAvail, newSchedule, workersLeft.tail, ridesNeedingOps.patch(i,Nil,1))
        }
      }
      recurByWorker(daysLeft, workerAvail, schedule, workersLeft.tail, ridesNeedingOps)
    }
  }

  def recurByDay(daysLeft:List[String], workerAvail:List[WorkerDays], schedule:List[WorkerAssigns]) {
    if (daysLeft.isEmpty) {
      ...
      recurByWorker(daysLeft.tail, workerAvail, WorkerAssigns(day, Nil)::schedule, workerAvail.map(_.name), operatorRides)
    }
  }

  recurByDay(days, employeeInfo.map(e => WorkerDays(e.name, e.daysPerWeek)), List[WorkerAssigns]())
}
...
def hireEmployee {
  ...
  val name = readLine()
  println("How many days can he/she work each week?")
  val dpw = readInt()
  employeeInfo ::= EmployeeData(name,Nil,dpw)
}
...
do {
 ...
 7) Quit.""")
  ...
  input match {
    ...
    case 4 => addRide
    case 5 => recursiveBuildWeeklySchedule
    case 6 => 
